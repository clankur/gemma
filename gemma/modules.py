...
class Attention(nn.Module):
    ...
    def setup(self):
        ...
        self.roped_k = Identity()
        self.scaled_q = Identity()
        self.reshaped_scaled_q = Identity()
        self.roped_q = Identity()
        self.att_logits = Identity()
        self.capped_logits = Identity()
        self.masked_logits = Identity()
        self.att_wei = Identity()
        self.a_out_premix = Identity()
        self.a_out = Identity()
    ...
    def __call__(
        ...
        query_proj = self.roped_q(query_proj)
        query_scaled = query_proj * self.query_pre_attn_scalar
        query_scaled = self.scaled_q(query_scaled)
        ...
        key_proj = self.roped_k(key_proj)
        ...
        if self.use_gqa:
            ...
            query_scaled = self.reshaped_scaled_q(query_scaled)
            # print(f"{kg=}, {self.num_kv_heads=}, {int(kg / self.num_kv_heads)=}")
        ...
        padded_logits = self.masked_logits(padded_logits)
        probs = jax.nn.softmax(padded_logits, axis=-1).astype(key_proj.dtype)
        probs = self.att_wei(probs)
        if self.use_gqa:
            # Reshape matrices to enable einsums over groups.
        ...
        encoded = self.a_out_premix(encoded)
        attn_output = self.attn_vec_einsum('BTNH,NHD->BTD', encoded)
        attn_output = self.a_out(attn_output)
        if cache is not None:
            new_cache = {
                'v': value_proj,
                'k': key_proj,
