...
class Transformer(nn.Module):
    ...
    def setup(self):
        ...
        self.tracked_embed = modules.Identity()
        self.tracked_unembed = modules.Identity()
        self.final_softcap = modules.Identity()
        self.blocks = [
            modules.Block(
                name=f'layer_{i}',
    ...
    def __call__(
        ...
        logits = self.tracked_unembed(logits)

        if self.config.final_logit_softcap is not None:
            logits /= self.config.final_logit_softcap
            logits = jnp.tanh(logits) * self.config.final_logit_softcap
        logits = self.final_softcap(logits)
